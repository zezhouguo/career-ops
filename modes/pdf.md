...
12. Build competency grid from JD requirements (6-8 keyword phrases) — draw from Step 4's `existing` and `supportedByResume` buckets only, NEVER from `gap`
...
18. **Fact gate (hard):** run `node verify-cv-facts.mjs output/cv-{candidate}-{company}.html`. It checks every metric-like claim in the generated HTML against `cv.md` + `article-digest.md` (plus verified exceptions in the gitignored user-layer `config/cv-facts.json`). Exit 1 is a HARD STOP before PDF rendering: remove the invented metric from the payload and rebuild via Step 17 — or, if the number is real but undocumented, ask the user to confirm and add the evidence to `cv.md`/`article-digest.md` (or an exception to `config/cv-facts.json`) first. Never ship a CV with an unverified number.
...
20. **Verify layout and ATS text-layer, respond to overflow (skip silently if the console says `pdftotext (poppler) is not on PATH` — proceed with whatever the page-count-only check already gave you):**
    ...
    c. **If the page count exceeds the cap or the visual check finds an overflow-driven defect**, work through this ladder, re-running Step 19's command and re-checking after each change, capped at 3-4 rounds before stopping to ask the user rather than looping indefinitely:
...
- **`publications[]` is the full list from cv.md.** `build-cv-html.mjs` has no truncation code path, so it can never shorten it. Overflow is resolved by the Step 20 ladder (density nudge, then Experience/Project bullet trim), never by cutting publications.
...
- `jd` is optional metadata for `cv-trim.mjs` (Step 20c): `keywords` mirrors Step 3's extracted list so the trim loop scores relevance from the same signal instead of re-deriving one; `text` (optional) supplies 1×-weight body tokens. Both builders ignore this field.
