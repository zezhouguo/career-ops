...
14. **Verify layout and ATS text-layer, respond to overflow** (skip silently if the JSON report's `textVerification.available` is `false` — `pdftotext`/poppler isn't installed; proceed with whatever page count you have):
    ...
    c. **On overflow or an overflow-driven visual defect**, work through this ladder, re-running Step 13 and re-checking after each change, capped at 3-4 rounds before stopping to ask the user:
       ...
       2. **Content trim (mechanized):** run `node cv-trim.mjs /tmp/cv-{candidate}-{company}.json --format=latex --max-pages={N} --out=output/cv-{candidate}-{company}-{YYYY-MM-DD}.pdf --jd-keywords={Step-4 keywords, comma-joined} [--achievements=<cover payload json>]` — same algorithm as the HTML path (3× keyword + 1× body relevance, uniqueness, last-bullet protection, `education[]`/`skills[]`/`publications[]` structurally untouchable, one bullet per iteration). Requires `tectonic`/`pdflatex` AND poppler on PATH (the loop must compile and count pages each round; it stops with a clear error otherwise). Read the JSON report (`cuts[]`, `coverDependentCut`, `trimmedPayload`) and record the trimmed payload as the CV's source; re-run Step 13 once at the end for the final verification report.
...
| `jd` | object | Optional — `{ "keywords": [], "text": "" }`; consumed by `cv-trim.mjs`'s trim loop (Step 14c), ignored by the builder. |
